...
import com.onix.scalecamerapreview.interfaces.RotationCallback;
import com.onix.scalecamerapreview.interfaces.RotationListener;
import com.onix.scalecamerapreview.models.Size;
import com.onix.scalecamerapreview.scale_strategy.CenterCropStrategy;
import com.onix.scalecamerapreview.scale_strategy.FitCenterStrategy;
...
public class CameraPreview extends ViewGroup {
    ...
    protected void initializeAttributes(AttributeSet attrs) {
        TypedArray styledAttributes = getContext().obtainStyledAttributes(attrs, R.styleable.scale_camera);

        int framingRectWidth = (int) styledAttributes.getDimension(R.styleable.scale_camera_framing_rect_width, -1);
        int framingRectHeight = (int) styledAttributes.getDimension(R.styleable.scale_camera_framing_rect_height, -1);

        if (framingRectWidth > 0 && framingRectHeight > 0) {
            this.framingRectSize = new Size(framingRectWidth, framingRectHeight);
        }

        int framingColor = styledAttributes.getColor(R.styleable.scale_camera_framing_color, Color.BLACK);
        setBackgroundColor(framingColor);

        this.useTextureView = styledAttributes.getBoolean(R.styleable.scale_camera_use_texture_view, true);

        // See zxing_attrs.xml for the enum values
        int scalingStrategyNumber = styledAttributes.getInteger(R.styleable.scale_camera_preview_scaling_strategy, -1);
